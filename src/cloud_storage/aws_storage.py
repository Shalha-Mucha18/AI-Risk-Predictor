...
class SimpleStorageService:
    ...
    def s3_key_path_available(self, bucket_name: str,s3_key: str) -> bool:
        ...
        try: 
            bucket: Bucket = self.s3_resource.Bucket(bucket_name)
    ...
    def get_bucket(self,bucket_name:str):
        ...
        """
        logging.info("Entered the get_bucket method")
        try:
            bucket = self.s3_resource.Bucket(bucket_name)
            logging.info("Exited the get_bucket method")
            return bucket
        except Exception as e:
            raise MyException(e, sys) from e    
    ...
    def get_file_object(self,filename:str,bucket_name:str):
        ...
        Returns:
            Object: S3 file object.

        """
        logging.info("Entered the get_file_object method")
        try:
            bucket = self.get_bucket(bucket_name)
            file_object = bucket.Object(filename)
            logging.info("Exited the get_file_object method")
            return file_object
        except Exception as e:
            raise MyException(e, sys) from e    
    ...
    def load_model(self,model_name:str,bucket_name:str,model_dir:str = None) -> object:
        ...
        """
        logging.info("Entered the load_model method")
        try:
    ...
    def create_folder(self,folder_name:str,bucket_name:str) -> None:
        ...
        """
        logging.info("Entered the create_folder method")
        try:
            # check if folder already exists
            self.s3_key_path_available(bucket_name,folder_name).load()
        except ClientError as e:
            # if folder does not exist, create it
            if e.response["Error"]["Code"] == "404":
                folder_obj = folder_name + "/"
                self.s3_client.put_object(Bucket=bucket_name, Key=folder_obj)
                logging.info("Exited the create_folder method")
            else:
                raise e    
        except Exception as e:
            raise MyException(e, sys) from e    
    ...
    def upload_file(self,from_filename: str, to_filename:str,bucket_name:str,remove:bool = True):
        """
        Uploads a file from the local file system to the specified S3 bucket.
        Args:
            from_filename (str): The local path to the file to upload.
        ...
        logging.info("Entered the upload_file method")

        try:
             logging.info(f"Uploading file {from_filename} to {to_filename} in bucket {bucket_name}")
             self.s3_resource.meta.client.upload_file(from_filename,bucket_name,to_filename)
             logging.info("Excited the upload file method")
             if remove:
                logging.info(f"Removing local file {from_filename}")
    ...
    def upload_df_as_csv(self,data_frame: DataFrame,local_filename:str,bucket_filename:str,remove:bool = True):
        ...
        Args:
            data_frame (Dataframe): The pandas DataFrame to upload.
            local_filename (str): The local path to save the CSV file before uploading.
            bucket_name (str): The name of the S3 bucket.
            remove (bool): Whether to remove the local file after successful upload.
        ...
        Returns:
            None
        """
        logging.info("Entered the upload_df_as_csv method")
        try:
            # save dataframe as csv
            data_frame.to_csv(local_filename,index=False,header=True)
            # upload csv to s3 bucket
            self.upload_file(local_filename,bucket_filename,bucket_name)
            logging.info("Exited the upload_df_as_csv method")
        except Exception as e:
            raise MyException(e, sys) from e   
    ...
    def read_csv(self,filename: str, bucket_name:str) -> DataFrame:
        ...
        """  
        logging.info("Entered the read_csv method")
        try: 
            csv_obj = self.get_file_object(filename,bucket_name)
            df = self.get_df_from_object(csv_obj)
            logging.info("Exited the read_csv method")
            return df
        except Exception as e:
            raise MyException(e, sys) from e          
